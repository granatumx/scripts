...
if [ -f "/tmp-gx/package.yaml" ]; then
  docker run --network=gx-net --rm -v gx-tmp:/tmp-gx -it granatumx/install-gbox-run:{VER} ./installGbox --gboxDir=/tmp-gx
fi

if [ -d "/tmp-gx/recipes" ]; then
  docker run --network=gx-net --rm -v gx-tmp:/tmp-gx -it granatumx/install-gbox-run:{VER} ./installGbox --recipesDir=/tmp-gx/recipes
fi
rm -rf /tmp-gx/*
