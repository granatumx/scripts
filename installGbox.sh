...
if [ -f "/tmp-gx/package.yaml" ]; then
  docker run --network=host --rm -v gx-tmp:/tmp-gx -it granatumx/gbox-install-run:1.0.0 ./installGbox --gboxDir=/tmp-gx
fi

if [ -d "/tmp-gx/recipes" ]; then
  docker run --network=host --rm -v gx-tmp:/tmp-gx -it granatumx/gbox-install-run:1.0.0 ./installGbox --recipesDir=/tmp-gx/recipes
fi
rm -rf /tmp-gx/*
